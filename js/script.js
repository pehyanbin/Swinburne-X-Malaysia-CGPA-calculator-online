...
// Available themes
const THEMES = ['light', 'dark', 'dark-blue', 'dark-green', 'dark-red', 'pink', 'purple', 'orange', 'high-contrast'];
...
function displayResults(results, cgpa, systemType) {
    ...
    // Set Comment
    document.getElementById('comment-container').textContent = getCGPAComment(cgpa);
...
/**
 * Get a warm-hearted comment based on the CGPA
 * @param {number} cgpa - The calculated CGPA
 * @returns {string} - The comment
 */
function getCGPAComment(cgpa) {
    if (cgpa >= 3.8) return "Outstanding achievement! Your hard work and dedication are truly shining through. Keep reaching for the stars! 🌟";
    if (cgpa >= 3.5) return "Fantastic work! You're doing incredibly well. Be proud of your efforts and keep that momentum going! ✨";
    if (cgpa >= 3.0) return "Great job! You're building a solid foundation. Trust in your abilities and keep moving forward with confidence! 💪";
    if (cgpa >= 2.5) return "Good effort! You're making progress. Remember, every step forward is a victory. Keep believing in yourself! 🌱";
    if (cgpa >= 2.0) return "You're doing okay! The journey of learning has its ups and downs. Stay positive and keep doing your best—you've got this! 🌈";
    return "Don't be discouraged. Grades don't define your worth or your future. Take a deep breath, stay persistent, and know that you have the potential to grow! ❤️";
}
